...
contract FairStaking is ReentrancyGuard, Ownable {
    ...
    function stake(uint256 _amount, uint _lockupPeriod) external nonReentrant {
        ...
        require(
            _lockupPeriod >= users[msg.sender].lockupPeriod,
            "Cannot decrease lockup period"
        );
    ...
    function unstake() external nonReentrant {
        ...
        require(
            block.timestamp >=
                users[msg.sender].stakingTimestamp +
                    lockupTimes[users[msg.sender].lockupPeriod],
    ...
    function calculateRewards(address _user) public view returns (uint256) {
        ...
        uint256 totalStakingDuration = stakingEndTimestamp -
            stakingStartTimestamp;
        uint256 userStakingDurationShare = (userStakingDuration * 1e9) /
        ...
        uint256 accrued = (userStakingDurationShare *
            userWeightShare *
            stakingRewardsTotal) / 1e18;
        if (accrued > user.paidOutRewards) return accrued - user.paidOutRewards;
        else return 0;
    }
    ...
    function apyForRatio(
        ...
        return
            (((rewards * 1e9) / balance) *
                ((SECONDS_IN_YEAR * 1e9) /
                    (stakingEndTimestamp - stakingStartTimestamp))) / 1e14;
    }
}
